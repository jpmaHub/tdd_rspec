...
class AddressBook 
def initialize(firstLine, city, postcode)
    @firstLine = firstLine
    @city = city
    @postcode = postcode
  end
 ...
 def getFull()
 "#{@firstLine} #{@city} #{@postcode}".split.join(" ")
 end 
end 
...
RSpec.describe Person do
  person_full = Person.new(first_name:"John",middle_name:"Lee",last_name:"Cooper")
  person_fl = Person.new(first_name:"John",last_name:"Cooper")
  add = AddressBook.new("345","London","EC1290")
  describe "#full_name" do

    it "concatenates first name, middle name, and last name with spaces" do
    
    expect( person_full.full_name()).to eq("John Lee Cooper")
    expect(person_full.getAdd(add.getFull)).to eq("John Lee Cooper 345 London EC1290")
    end 
    ...
    it "add contact with name" do
     expect(person_fl.getAdd(add.getFull)).to eq("John Cooper 345 London EC1290")
  end 
  end
